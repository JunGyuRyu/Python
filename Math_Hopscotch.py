...
#BGM On/Off 버튼 만드는 클래스

def myFunction():
    root = tk.Tk()
    root.title("BGM 설정")
    root.geometry("250x100+650+380") #가로 x 세로 + 창 위치좌표
...
def runGame():
    ...
    item_index = 0
    item = 0
    myFont = pygame.font.Font( None, 30) #font객체 생성
    ...
    RNumSrn = [[0]*9 for x in range(9)] #9x9 행렬 만들고
    for columns in range(COLUMN_COUNT):
            for rows in range(ROW_COUNT):
                    RNumSrn[columns][rows]=r.randint(1,10) #1~10의 랜덤 숫자를 9x9행렬에 초기화
    while not done:
                ...
                if turn == 0:
                        ...
                        if event.type == pygame.QUIT:
                            ...
                            if event.key in KEY_DIRECTION:
                                ...
                                turnCount += 1
                                P2sound.play()
        #턴 구분해서 방향키 누르면 밟은 곳 숫자만큼 점수에 더함
